(()=>{
	...
	//core.cursor为常量
    core.cursor="url(./img/cursor.ico),auto";
    core.som={};
	(()=>{
		...
		core.bgm={
			...
			play:function(name){
				...
				if(bgm.src!=="./music/"+name)bgm.src="./music/"+name;
	...
	core.REMstring=function(inp){
		...
		//从导入的图片列表中查找，若有，则输出
		for(let name of core.loadlist.loadimage){
	...
	core.setDrag=function(ele,layer){
		...
		ele.on('pointerdown', (event) => {
			Cursor="url(./img/move.ico),pointer";
			isDragging = true;
	...
	core.importImage=function(){//开发者使用，自动导入img文件夹内的png图片并存入img.json
		...
		fs.readdir("./img",(err,data)=>{
			if(err){
				...
				fs.writeFile("./img/img.json",JSON.stringify(imglist,null,"	"),"utf-8",()=>{console.log("导入成功");});
			}
		});
	}
	...
	core.setSlide=function(ele,layer,points,actions={}){
		...
		ele.on("pointerover",()=>{Cursor="url(./img/move.ico),pointer";});
