(async()=>{
	const setting={
		...
		在LoadingFont字体未加载时，会显示"./img/loading.png"作为代替项。
	...
	//加载的英文字体
	let loadEnFont=[
		["Brianneshand","Brianneshand"]//文件名，引用名
	];
	...
	const loadimage=await(await fetch('./img/img.json')).json();

	//加载的中文字体（有版权争议，须声明以免责）
	const cnfont=await(await fetch('./Font/font.json')).json();
	...
	//加载英文字体
	let lef=(fo,r)=>{
		const Font = new FontFace(fo[1], "url(./enFont/"+fo[0]+".ttf)");
		lf(fo[1],Font,r);
	}
	//加载中文字体
	let lcf=(fp,r)=>{
		if(fp.url==="default"){r();return;}
		const Font = new FontFace(fp.name,fp.url);
		lf(fp.name,Font,r);
	}
	//加载动画JSON文件的函数
	core.aniJSON=async function(name){
		const response = await fetch('./animate/'+name+'.json');
	...
	for(let i=0;i<cnfont.packages.length;i++){
		let f=cnfont.packages[i];
	...
	loadtext("正在加载图片");
	for(let im of loadimage){
		tasks.im.push(PIXI.Assets.load("./img/"+im+".png").then(timg=>{
			core.img[im]=timg;
		}));
